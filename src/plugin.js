  ...
  return (store) => {
    Object.keys(models).forEach((key) => {
      ...
      store.registerModule(key, {
        ...
        actions: {
          [`${key}.fetch`]: ({ state, commit }) => axios.get(`/${key}`).then((response) => {
            const results = [];
          ...
          [`${key}.create`]: ({ state, commit }, data) => axios.post(`/${key}`, data).then((response) => {
            commit(`${key}.add`, response.data);
            return state[key][response.data.id];
          }),
          [`${key}.update`]: ({ state, commit }, data) => axios.put(`/${key}/${data.id}`, data).then((response) => {
            commit(`${key}.update`, response.data);
            return state[key][response.data.id];
          }),
          [`${key}.get`]: ({ state, commit }, id) => axios.get(`/${key}/${id}`).then((response) => {
            commit(`${key}.update`, response.data);
            return state[key][response.data.id];
          }),
          [`${key}.delete`]: ({ state, commit }, id) => axios.delete(`/${key}/${id}`).then((response) => {
